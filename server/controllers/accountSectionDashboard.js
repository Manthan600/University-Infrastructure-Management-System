...
exports.accountsBillApproval = async (req, res) => {
    try {
        ...
        if (user_type == "accounts") {
            const checkBillQuery = `SELECT tech_id FROM bills WHERE bill_id = ? AND admin_approval = true`;
            connection.query(checkBillQuery, [bill_id], async (err, billResults) => {
                ...
                connection.query(fetchTechnicianEmailQuery, [tech_id], async (err, technicianResults) => {
                    ...
                    const approveBillQuery = `UPDATE bills SET acc_sec_approval = true WHERE bill_id = ?`;
                    ...
                    connection.query(approveBillQuery, [bill_id], async (err, results) => {
                        ...
                        try {
                            ...
                            const emailContent = `Bill with ID ${bill_id} has been approved by the accounts section.`;
                            ...
                            return res.status(200).json({
                                message: `Bill with ID ${bill_id} approved successfully from accounts section. Email sent to technician.`,
                            });
                        } catch (emailError) {
