...
export default function Faculty() {
  ...
  const handleApproveComplaint = async (token_id) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/approveComplaint', { token_id, device_type: 'computer', user_type: 'admin' });
  ...
  const deleteComplaint = async (token_id) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/deleteComplaint', { token_id, device_type: 'computer', user_type: 'admin' });
  ...
  return (
    ...
    {comps.map(device => (
      <tr key={device.device_id}>
        ...
        <td className="text-center">
          ...
          <button onClick={(e) => { e.preventDefault(); }} type="submit" className="btn btn-danger">Delete</button>
        </td>
      </tr>
    ))}
                  </tbody>
    ...
    {student.map(staffMember => (
      <tr key={staffMember.MIS}>
        ...
        <td className="text-center">
          <Link to={`/updatestaff/${staffMember.id}`} style={{ marginRight: '10px' }} className="btn btn-primary">Update</Link>
          <button onClick={(e) => { e.preventDefault();  }} type="submit" className="btn btn-danger">Delete</button>
        </td>
      </tr>
    ))}
  ...
  <tr key={technician.MIS}>
    ...
    {/* Add more columns if needed */}
    <td className="text-center">
      <Link to={`/updatetechnician/${technician.MIS}`} style={{ marginRight: '10px' }} className="btn btn-primary">Update</Link>
      <button onClick={(e) => { e.preventDefault(); /* Handle delete functionality */ }} type="submit" className="btn btn-danger">Delete</button>
    </td>
  </tr>
))}
