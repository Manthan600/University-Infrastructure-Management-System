...
export default function Faculty() {
  ...
  const getAllComplaintsAdmin = async (deviceType) => {
    try {
      const response = await axios.get('http://localhost:4000/api/v1/getAllComplaintsAdmin', {
        params: {
  ...
  const handleApproveComplaint = async (token_id) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/approveComplaint', {
        token_id: token_id,
  ...
  const deleteComplaint = async (token_id) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/deleteComplaint', {
        token_id: token_id,
        device_type: selectedDeviceType,
        user_type: sessionStorage.getItem('user_type')
      });
  ...
  const deleteComp = async (device_id) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/deleteComp', {
        device_id: device_id,
  ...
  const deleteProj = async (device_id) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/deleteProj', {
        device_id: device_id,
  ...
  const deleteAc = async (device_id) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/deleteAc', {
        device_id: device_id,
        device_type:"ac",
        user_type: sessionStorage.getItem('user_type')
      });
  ...
  const getAllDevices = async () => {
    try {
      const response = await axios.get('http://localhost:4000/api/v1/getAllDevices', {
        params: {
          user_type: sessionStorage.getItem('user_type')
        }
      });
      ...
      const compData = data.filter(device => device.device_type === 'computer');
      const projectorData = data.filter(device => device.device_type === 'projector');
  ...
  const getAllUsers = async () => {
    try {
      const response = await axios.get('http://localhost:4000/api/v1/getAllUsers');
      const { data } = response.data;
  ...
  const deleteStudent = async (MIS) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/deleteStudent', {  MIS });
  ...
  const deleteAcc = async (username) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/deleteAcc', { username });
      // console.log('User Deleted:', response.data);
      setShowAlertDel(true);
      getAllUsers();
      setTimeout(() => {
  ...
  const deleteTech = async (tech_id) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/deleteTech', { tech_id });
      // console.log('User Deleted:', response.data);
      setShowAlertDel(true);
      getAllUsers();
      setTimeout(() => {
  ...
  const deleteAdmin = async (username) => {
    try {
      const response = await axios.post('http://localhost:4000/api/v1/deleteAdmin', { username });
      // console.log('User Deleted:', response.data);
      setShowAlertDel(true);
      getAllUsers();
      setTimeout(() => {
  ...
  return (
    <div>
      ...
      {selectedOption === 'complaint' && (
        <div>
          ...
          <div className="box">
            ...
            <label htmlFor="">Select Device Type:</label>
              <select onChange={(e) => handleDeviceTypeChange(e.target.value)} className="form-select" aria-label="Default select example">
                    ...
                    {unverifiedComplaints.map(complaint => (
                      <tr key={complaint.token_id}>
                        ...
                        <td className="text-center">
                          ...
                          <button onClick={(e) => { e.preventDefault(); handleApproveComplaint(complaint.token_id) }} class="btn btn-success" style={{ marginRight: '10px' }}>Verify</button>
                  ...
                  <tbody>
                    {notAcceptedComplaints.map(complaint => (
                      <tr key={complaint.token_id}>
                        ...
                        <td>{new Date(complaint.complaint_date).toISOString().split('T')[0]}</td>
                        <td className="text-center">
                          <button onClick={(e) => { e.preventDefault(); deleteComplaint(complaint.token_id) }} className="btn btn-danger">Delete</button>
                        </td>
                      </tr>
                    ))}
                  ...
                  <tbody>
                    ...
                    {comps.map(device => (
                      <tr key={device.device_id}>
                        ...
                        <td>{new Date(device.DOI).toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' })}</td>
                        <td>{device.status}</td>
                        <td className="text-center">
                          <button onClick={(e) => { e.preventDefault(); deleteComp(device.device_id)}} type="submit" className="btn btn-danger">Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <tbody>
                    {projectors.map(device => (
                      <tr key={device.device_id}>
                        ...
                        <td>{device.status}</td>
                        <td className="text-center">
                          <button onClick={(e) => { e.preventDefault();deleteProj(device.device_id) }} type="submit" className="btn btn-danger">Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <tbody>
                    ...
                    {ac.map(device => (
                      <tr key={device.device_id}>
                        ...
                        <td>{device.status}</td>
                        <td className="text-center">
                          <button onClick={(e) => { e.preventDefault();deleteAc(device.device_id) }} type="submit" className="btn btn-danger">Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <tbody>
                    {student.map(staffMember => (
                      <tr key={staffMember.MIS}>
                        ...
                        <td className="text-center">
                          <button onClick={(e) => { e.preventDefault(); deleteStudent(staffMember.MIS) }} type="submit" className="btn btn-danger">Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <tbody>
                    ...
                    {tech.map(technician => (
                      <tr key={technician.MIS}>
                        ...
                        {/* Add more columns if needed */}
                        <td className="text-center">
                          <button onClick={(e) => { e.preventDefault(); deleteTech(technician.MIS) }} type="submit" className="btn btn-danger">Delete</button>
                        </td>
                      </tr>
                    ))}
                  ...
                  <tbody>
                    ...
                    {admin.map(admin => (
                      <tr key={admin.MIS}>
                        ...
                        {/* Add more columns if needed */}
                        <td className="text-center">
                          <button onClick={(e) => { e.preventDefault();  deleteAdmin(admin.MIS)}} type="submit" className="btn btn-danger">Delete</button>
                        </td>
                      </tr>
                    ))}
                  ...
                  <tbody>
                    ...
                    {account_section.map(Acc => (
                      <tr key={Acc.MIS}>
                        ...
                        {/* Add more columns if needed */}
                        <td className="text-center">
                          <button onClick={(e) => { e.preventDefault();  deleteAcc(Acc.MIS) }} type="submit" className="btn btn-danger">Delete</button>
                        </td>
                      </tr>
                    ))}
                  ...
                  <tbody>
                    {resolvedComplaints.map(complaint => (
                      <tr key={complaint.token_id}>
                        ...
                        <td>{new Date(complaint.resolved_date).toISOString().split('T')[0]}</td>
                        <td>{complaint.tech_id}</td>
                        <td style={{ color: 'blue' }}><strong>For payment contact Acc Section</strong></td>
      ...
      <Alert show={showAlertDel} variant="danger"  style={{
